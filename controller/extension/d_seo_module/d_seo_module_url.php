		...
		if ($status) {			
			...
			if (isset($this->request->get['route'])) {
				if ($this->request->get['route'] == 'product/category') {
					...
					if ($category_id) {		
						$category_path = $this->{'model_extension_d_seo_module_' . $this->codename}->getCategoryPath($category_id, $setting['cache_expire']);
					...
					if ($product_id) {
						$product_path = $this->{'model_extension_d_seo_module_' . $this->codename}->getProductPath($product_id, $setting['cache_expire']);
						
						if ($setting['sheet']['product']['unique_url']) {							
							$url_data = array();
							
							if (!$setting['sheet']['product']['short_url']) {
								$url_data['path'] = $product_path;
							}
		...
		if ($status) {
			...
			if ($setting['redirect']['status']) {					
				...
				if ($url_to) {
					...
					if ($url_to_info['host'] . $url_to_info['port'] . $url_to_info['path'] != $url_from_info['host'] . $url_from_info['port'] . $url_from_info['path']) {
	...
	public function rewrite($url) {
		...
		if (isset($url_info['data']['route']) && $this->registry->has('d_cache')) {
			...
			if ($url_info['data']['route'] == 'product/category') {
				...
				if ($category_id) {
					$url_rewrite = $this->d_cache->get($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $category_id . '.' . $store_id . '.' . $language_id);
					...
					if ($url_rewrite) {
						...
						$this->d_cache->set($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $category_id . '.' . $store_id . '.' . $language_id, $url_rewrite);
				...
				if ($product_id) {
					$url_rewrite = $this->d_cache->get($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $product_id . '.' . $store_id . '.' . $language_id);
					...
					if ($url_rewrite) {
						...
						$this->d_cache->set($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $product_id . '.' . $store_id . '.' . $language_id, $url_rewrite);
				...
				if ($manufacturer_id) {
					$url_rewrite = $this->d_cache->get($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $manufacturer_id . '.' . $store_id . '.' . $language_id);
					...
					if ($url_rewrite) {
						...
						$this->d_cache->set($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $manufacturer_id . '.' . $store_id . '.' . $language_id, $url_rewrite);
				...
				if ($information_id) {
					$url_rewrite = $this->d_cache->get($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $information_id . '.' . $store_id . '.' . $language_id);
					...
					if ($url_rewrite) {
						...
						$this->d_cache->set($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $information_id . '.' . $store_id . '.' . $language_id, $url_rewrite);
				...
				if (!in_array($url_info['data']['route'], $setting['custom_page_exception_routes'])) {
					$url_rewrite = $this->d_cache->get($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $store_id . '.' . $language_id);
					...
					if ($url_rewrite) {
						...
						$this->d_cache->set($this->codename, 'url_rewrite.' . md5($url_info['data']['route']) . '.' . $store_id . '.' . $language_id, $url_rewrite);
				...
				if ($this->request->get['route'] == 'product/category') {
					...
					if ($category_id) {
						...
						if (isset($this->request->get['page']) && $this->request->get['page'] > 1) {
							$args .= '&page=' . urlencode(html_entity_decode($this->request->get['page'], ENT_QUOTES, 'UTF-8'));
						}
					}
				} elseif ($this->request->get['route'] == 'product/product') {
					if (isset($this->request->get['product_id'])) {
					...
					if (isset($this->request->get['search'])) {
						$args .= '&search=' . urlencode(html_entity_decode($this->request->get['search'], ENT_QUOTES, 'UTF-8'));
					}

					if (isset($this->request->get['tag'])) {
						$args .= '&tag=' . urlencode(html_entity_decode($this->request->get['tag'], ENT_QUOTES, 'UTF-8'));
					}
